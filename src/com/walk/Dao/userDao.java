...
public class userDao {
	...
	public jsonMessage loginDao(String userName,String password) throws Exception{
		...
		jsonMessage jm=null;
		if(r.next()) {
			String sql1="SELECT * from tblUserInfor where userName='"+userName+"'";
	...
	public jsonMessage registerDao(String userName,String password,String phone) throws Exception {
		...
		if(result.equals("success")) {
		    ...
		    pstmt.setString(5, "http://172.17.145.90:8080/WorldWalk/image/tx.png");
		    pstmt.setString(6, "男");
		    pstmt.setString(7, "0");
		    pstmt.executeUpdate();  			
	...
	//修改个人信息
	public jsonMessage modifyUserDataDao(String userID,String path,MultipartFile part,String nickName,String phone,String sex) {
		...
		try {
            ...
            ps.setString(3, sex);
            ps.setString(4, "http://172.17.145.90:8080/WorldWalk/image/"+userID+".png");
            ps.executeUpdate();  
            jm=new jsonMessage("true","修改成功");
            ps.close();
		}catch(Exception e) {
	...
	public jsonMessage updateUserStateDao(String userID,String Longititude,String Latitude,String Direction) {
		...
		try {
			...
			stmt.close();
			if(r.next()) {
				String sql2="update tblGps set userID=?,Longititude=?,Latitude=?,Direction=? where userID='"+userID+"'";
				...
				String sql3="insert into tblGps(userID,Longititude,Latitude,Direction) values(?,?,?,?)";
